...
class AudioReceiver():
    ...
    def get_loaded_samples(self):
        """
        Returns a copy of the full in-memory mono waveform for the currently
        loaded file (mode == "file"), or None if no file has been loaded yet.
        Use this instead of accessing _file_samples directly.
        """
        with self._transport_lock:
            if self._file_samples is None:
                return None
            return self._file_samples.copy()
