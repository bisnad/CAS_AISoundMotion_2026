"""
stableaudio3_gui_text_to_audio.py

GUI for text-to-audio and init_audio generation, with model size/variant
switching. ALL Stable Audio 3-specific logic (model loading, audio I/O,
generation) lives in stable_audio_3_core.py and is imported here -- this
file contains ONLY GUI code: Qt widgets, waveform plotting, playback
device handling, and the QThread workers that call into the core module.

Compare stable_audio_3_core.py's generate_text_to_audio() directly against
hello_stable_audio_3.py's Section 1/2 -- they are the same logic, just
factored into a reusable function instead of inline script code.
"""
...
# ----------------------------------------------------------------------
# GUI-only constants (nothing Stable-Audio-3-specific here)
# ----------------------------------------------------------------------
...
def tensor_to_playback_numpy(audio_tensor):
    """GUI-only: converts a (channels, samples) tensor to the
    (samples, channels) float32 numpy layout sounddevice expects."""
    arr = audio_tensor.detach().cpu().numpy().astype(np.float32)
    return np.ascontiguousarray(arr.T)


def compute_minmax_envelope(mono_samples, target_points):
    """GUI-only: downsamples a signal to a min/max envelope for fast
    waveform rendering -- has nothing to do with Stable Audio 3 itself."""
    n = len(mono_samples)
    if n <= target_points:
        return None
...
# ============================================================================
# QThread workers -- thin wrappers around stable_audio_3_core functions.
# No Stable-Audio-3-specific logic lives here; every worker's run() body
# is a direct call into the core module.
# ============================================================================

class LoadModelWorker(QThread):
    finished = pyqtSignal(object, object, int, int, str, str)  # model, conditioner, sr, io_ch, device, variant
...
class GenerateWorker(QThread):
    ...
    def run(self):
        try:
            audio = sa3.generate_text_to_audio(**self.generate_kwargs)
...
# ============================================================================
# Waveform widget -- purely a Qt/pyqtgraph component, no Stable Audio 3
# awareness at all (it operates on plain tensors/sample rates handed to it).
# ============================================================================

class WaveformView(pg.PlotWidget):
    """Interactive waveform plot with left-drag region creation,
    click-to-select, and a live playhead line during playback."""
    ...
    def __init__(self):
        ...
        self.playhead_line = pg.InfiniteLine(
            pos=0.0, angle=90, movable=False, pen=pg.mkPen(color=(20, 20, 20), width=2),
        )
    ...
    def _install_drag_override(self):
        ...
        def custom_drag_event(ev, axis=None):
            ...
            if ev.isStart():
                ...
                self._drag_temp_region = pg.LinearRegionItem(
                    values=[start_x, current_x], brush=pg.mkBrush(100, 200, 100, 80),
                )
    ...
    def add_region(self, start_sec, end_sec):
        region = pg.LinearRegionItem(
            values=[start_sec, end_sec], brush=pg.mkBrush(*REGION_BRUSH_NORMAL), bounds=[0, self.duration_sec],
        )
        region.setZValue(10)
...
# ============================================================================
# Main window -- builds widgets, wires signals, and translates widget
# state into calls to stable_audio_3_core functions. No generation logic
# lives here directly.
# ============================================================================

class StableAudio3Gui(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Stable Audio 3 -- Text-to-Audio / Init-Audio GUI")
        ...
        self.init_audio_tensor = None
        self.generated_audio_tensor = None
    ...
    def _build_ui(self):
        ...
        outer.addWidget(self._build_prompt_group())
        outer.addWidget(self._build_init_audio_group())
    ...
    def _build_init_audio_group(self):
        group = QGroupBox(
            "Init audio -- left-drag to select the region used as init_audio "
            "(no selection = full file used)"
        )
        layout = QVBoxLayout()

        row = QHBoxLayout()
        self.init_audio_path_edit = QLineEdit()
        self.init_audio_path_edit.setReadOnly(True)
        self.init_audio_path_edit.setPlaceholderText("No file loaded -- plain text-to-audio will be used")
        browse_btn = QPushButton("Load .wav...")
        browse_btn.clicked.connect(self._on_browse_init_audio)
        clear_btn = QPushButton("Clear")
        clear_btn.clicked.connect(self._on_clear_init_audio)
        row.addWidget(self.init_audio_path_edit)
        row.addWidget(browse_btn)
        row.addWidget(clear_btn)
        layout.addLayout(row)

        self.init_waveform_view = WaveformView()
        layout.addWidget(self.init_waveform_view)
        self.init_waveform_view.region_selected.connect(self._on_init_region_selected)

        self.init_selection_label = QLabel("Init audio source: full file (no region selected)")
        layout.addWidget(self.init_selection_label)

        btn_row = QHBoxLayout()
        self.play_init_btn = QPushButton("Play init audio (region if selected, else full)")
        self.play_init_btn.clicked.connect(self._on_play_init_audio)
        clear_regions_btn = QPushButton("Clear regions")
        clear_regions_btn.clicked.connect(self.init_waveform_view.clear_regions)
        btn_row.addWidget(self.play_init_btn)
        btn_row.addWidget(clear_regions_btn)
        layout.addLayout(btn_row)

        noise_row = QHBoxLayout()
        noise_row.addWidget(QLabel("init_noise_level:"))
        self.init_noise_spin = QDoubleSpinBox()
        self.init_noise_spin.setRange(0.0, 1.0)
        self.init_noise_spin.setSingleStep(0.01)
        self.init_noise_spin.setValue(0.40)
        self.init_noise_spin.setToolTip("0.0 = unchanged source audio, 1.0 = pure noise (source ignored).")
        noise_row.addWidget(self.init_noise_spin)
    ...
    def _build_result_waveform_group(self):
        ...
        self.play_result_btn.clicked.connect(self._on_play_result_audio)
        self.play_result_btn.setEnabled(False)
        clear_regions_btn = QPushButton("Clear regions")
        clear_regions_btn.clicked.connect(self.result_waveform_view.clear_regions)
        self.save_btn = QPushButton("Save result...")
        self.save_btn.clicked.connect(self._on_save_generated_audio)
    ...
    def _build_action_row(self):
        ...
        self.stop_playback_btn.clicked.connect(self._on_stop_playback)
        row.addWidget(self.stop_playback_btn)
        return row

    # ------------------------------------------------------------------
    # Model selection / loading -- reads widget state, calls into core
    # ------------------------------------------------------------------
    ...
    def _apply_variant_defaults(self, variant):
        if variant == "base":
            ...
            self.cfg_scale_spin.setValue(1.0)
            self.cfg_scale_hint_label.setText(
                "Post-trained checkpoint: does not rely on CFG at inference. "
    ...
    def _on_browse_init_audio(self):
        path, _ = QFileDialog.getOpenFileName(self, "Load init audio", "", "WAV files (*.wav)")
        if not path:
            return
        ...
        self.init_audio_tensor = audio
        self.init_audio_path_edit.setText(path)
        self.init_waveform_view.set_waveform(audio, self.sample_rate)
        self.init_selection_label.setText("Init audio source: full file (no region selected)")
        self.status_bar.showMessage(f"Loaded init audio: {path}")

    def _on_clear_init_audio(self):
        self.init_audio_tensor = None
        self.init_audio_path_edit.clear()
        self.init_waveform_view.clear()
        self.init_selection_label.setText("Init audio source: full file (no region selected)")

    def _on_init_region_selected(self, region):
        if region is None:
            self.init_selection_label.setText("Init audio source: full file (no region selected)")
        else:
            start, end = region.getRegion()
            self.init_selection_label.setText(
                f"Init audio source: region [{start:.2f}s, {end:.2f}s] ({end - start:.2f}s)"
            )

    def _get_init_audio_slice(self):
        if self.init_audio_tensor is None:
            return None
        bounds = self.init_waveform_view.get_selected_bounds()
        if bounds is None:
            return self.init_audio_tensor
        start_sec, end_sec = bounds
        start_idx = max(0, int(start_sec * self.sample_rate))
        end_idx = min(self.init_audio_tensor.shape[1], int(end_sec * self.sample_rate))
        if end_idx <= start_idx:
            return self.init_audio_tensor
        return self.init_audio_tensor[:, start_idx:end_idx]

    # ------------------------------------------------------------------
    # Playback devices and playhead tracking (GUI-only)
    # ------------------------------------------------------------------
    ...
    def _play_numpy_audio(self, numpy_audio, sample_rate, view=None, offset_sec=0.0, duration_sec=0.0):
        if sd is None:
            QMessageBox.warning(
                ...
                "The 'sounddevice' package is not installed. Install it with 'pip install sounddevice'."
            )
            return
    ...
    def _on_play_init_audio(self):
        if self.init_audio_tensor is None:
            QMessageBox.information(self, "No init audio", "Load an init audio file first.")
            return
        bounds = self.init_waveform_view.get_selected_bounds()
        if bounds is None:
            segment, offset_sec = self.init_audio_tensor, 0.0
            ...
            if end_idx <= start_idx:
                return
            segment = self.init_audio_tensor[:, start_idx:end_idx]
        ...
        self._play_numpy_audio(
            ...
            view=self.init_waveform_view, offset_sec=offset_sec, duration_sec=duration_sec,
        )

    def _on_play_result_audio(self):
        if self.generated_audio_tensor is None:
            QMessageBox.information(self, "No result", "Generate audio first.")
            return
        bounds = self.result_waveform_view.get_selected_bounds()
        if bounds is None:
            segment, offset_sec = self.generated_audio_tensor, 0.0
            duration_sec = segment.shape[1] / self.sample_rate
        else:
            start_sec, end_sec = bounds
            start_idx = max(0, int(start_sec * self.sample_rate))
            end_idx = min(self.generated_audio_tensor.shape[1], int(end_sec * self.sample_rate))
            if end_idx <= start_idx:
                return
            segment = self.generated_audio_tensor[:, start_idx:end_idx]
            offset_sec, duration_sec = start_sec, end_sec - start_sec
        self._play_numpy_audio(
            tensor_to_playback_numpy(segment), self.sample_rate,
    ...
    def _on_generate_clicked(self):
        ...
        if not prompt:
            QMessageBox.warning(self, "Missing prompt", "Enter a text prompt before generating.")
            return
        ...
        init_slice = self._get_init_audio_slice()
        ...
        generate_kwargs = dict(
            ...
            cfg_scale=cfg_scale,
            init_audio_tensor=init_slice.to(self.model_device) if init_slice is not None else None,
            init_audio_sample_rate=self.sample_rate if init_slice is not None else None,
            init_noise_level=float(self.init_noise_spin.value()) if init_slice is not None else None,
        )

        self.status_bar.showMessage(f"Generating with model='{self.current_model_variant}' ...")
        self.progress_bar.setVisible(True)
        self.generate_btn.setEnabled(False)

        self.gen_worker = GenerateWorker(generate_kwargs, self.sample_rate)
        self.gen_worker.finished.connect(self._on_generation_finished)
        self.gen_worker.failed.connect(self._on_generation_failed)
        self.gen_worker.start()

    def _on_generation_finished(self, audio, sample_rate):
        self.generated_audio_tensor = audio
        self.progress_bar.setVisible(False)
        self.generate_btn.setEnabled(True)
        self.play_result_btn.setEnabled(True)
        self.save_btn.setEnabled(True)
        self.result_waveform_view.set_waveform(audio, sample_rate)
    ...
    def _on_save_generated_audio(self):
        if self.generated_audio_tensor is None:
            QMessageBox.information(self, "No generated audio", "Generate audio first.")
            return
        path, _ = QFileDialog.getSaveFileName(self, "Save generated audio", "generated.wav", "WAV files (*.wav)")
        if not path:
            return
        try:
            sa3.save_wav(path, self.generated_audio_tensor, self.sample_rate)
        except Exception as exc:
            QMessageBox.critical(self, "Save failed", str(exc))
            return
        self.status_bar.showMessage(f"Saved generated audio to {path}")
